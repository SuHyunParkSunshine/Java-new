...
//Number의 자식들, Integer, Double, Float, Long
public class Matrix<T extends Number> {

	private int rows;
	private int cols;
	// 제네릭 타입은 new로 생성자 생성 불가
	private T[][] data;
	 ...
	 * 생성자.
	 * <p>
	 * 지정한 행과 열의 개수의 인스턴스를 생성한다. 모든 원소의 값들은 지정한 Generic 타입의 임의의 값으로 설정됨.
	 * <p>
	...
	/**
	 * 인스턴스의 행렬 원소들을 Generic 타입에 맞는 임의의 값으로 설정한다. 임의의 값의 범위는 0 ~ Generic 타입 최대값의
	 * 제곱근.
	 * <p>
	...
	/**
	 * 인스턴스의 행렬 데이터를 터미널에 출력. 행렬의 Generic 타입을 먼저 출력 한 후 각각의 원소를 행과 열에 맞춰 출력한다.
	 */
	...
	/**
	 * 함수를 호출하는 인스턴스의 원소값에 파라미터로 제공되는 행렬의 원소값을 더함. 예 : {@code a.AddMatrix(b) } 는
	 * {@code a = a + b } 와 같은 결과.
	 * 
