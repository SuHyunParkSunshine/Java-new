...
class Sets {
	...
	{
		n = sz;
		parent = new int[sz + 1]; // Don't want to use parent[0] => Size of the Array
	...
	// parent[i]=-count[i] and parent[j]=-count[j].
	{
		...
		if (parent[i] > parent[j]) { // i has fewer nodes
			parent[i] = j; //i가 j에 달라 붙었다. 적은 거가 많은 거에 붙는게 더 효율적임
			parent[j] = temp;
...
public class TreeSet {
	...
	Sets s1 = new Sets(20);
	s1.SimpleUnion(0, 1);s1.SimpleUnion(1, 2);s1.SimpleUnion(3,4);s1.SimpleUnion(5,6);
